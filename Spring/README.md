...
4.  Do the above project using spring boot: Done.
...
5.  Make a JDBC project using Spring: Done.
...
6.  Create a Simple MVC WebApp using Spring Boot: Failed.

    (This uses a JSP to create a view to be returned when a link is accessed.)

    Create a Maven Spring Boot project with "Spring Web" as a dependency.

    Create a "webapp" folder and create an index.jsp file to create an Front-end that Java can work with.

    Create a controller to access this web app

7.  Create a REST project:

    Use RequestMapping("<path>") on a function to map that function to the url.

        GetMapping(), PostMapping(), etc are also available to map.

    use @ResponseBody on the requestmapped function and return a data type.

    If the data type has getters, setters, toStrings, AllArgumentsConstructor, NoArgumentConstructor, then it will send JSON of the object as a response.

    the above getter, setters etc can be accomplished using lombdok annotations @Getter, @Setter, etc.,

    if it is not annotated as a response body, the controller will look for a html/jsp page to send.

    Use @ResponseBody on any arguments for the function if the function is getting input from Post.

    Use @PathVariable to take inputs that aren't data types and are passed through the url:
    ...
    Use produces and consumes properties in the @Mapping to specify what types of datas can be transferred.
